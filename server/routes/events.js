const { Router } = require("express");
    ...
    router.put(
      ...
      [
        ...
        check("title", "Title is required").not().isEmpty(),
        check("title", "Title length must be max 32 characters").isLength({max: 32,}),
        check("start", "Start date is required").not().isEmpty(),
        check("start", "Invalid start date").custom(isDate),
        check("end", "End date is required").not().isEmpty(),
        check("end", "Invalid end date").custom(isDate),
        check("notes", "Notes length must be max 128 characters")
          .optional()
          .isLength({max: 128,}), validateFields, eventExistsById, isEventOwner,],
      updateEvent
    );
    ...
    router.delete(
      "/:id",
      [
        check("id", "Invalid event ID.").isMongoId(), validateFields, eventExistsById,
        isEventOwner,
      ],
      deleteEvent
    );
